...
class UserSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        for url in self.start_urls:
            ...
            user_detail_item['date_joined'] = selector.xpath('//div[@class="_UuSG _3dGg1 _2mtbj"]/span/text()').get()

            # User name
            user_detail_item['user_name'] = selector.xpath('//div[@class="_UuSG Naf1t"]/span[2]/text()').get()

            # User Image
            user_detail_item['user_image'] = selector.xpath('//div[@class="_UuSG _3mBt0 _1FVXP _1lNuM _2I3eh"]/img/@src').get()

            # Cover photo
            user_detail_item['cover_photo'] = selector.xpath('//div[@class="_UuSG _2Ap7s w77Za _3F-hn jpgPp _2z1u_"]/img/@src').get()
            ...
            row_no_post = selector.xpath('//span[@class="_2FoTG _UuSG _3_54N _2ZzNB _3hcKE L4pn5 _1XpDY"]')
            if not row_no_post:
                ...
                for _ in range(1, 31):
                    ...
                    # Wait to load page
                    time.sleep(get_project_settings().get('GAB_SCROLL_POST_PAUSE_TIME'))
                ...
                for page in pages:
                    ...
                    for row in row_post:
                        # Exclude reposts
                        row_repost = row.xpath('.//div[contains(@aria-label, "reposted")]')
                        if row_repost:
                            continue

                        # Post unique id
                        post = UserPostItem()
                        post['id'] = row.xpath('.//div[@class="_UuSG _2z1u_"]/div/@data-id').get()

                        # Post date and time
                        datetime_value = row.xpath('.//time/@datetime').get()
                        datetime_value_string = datetime_value if datetime_value else ''
                        post['datetime'] = datetime.strptime(datetime_value_string, '%Y-%m-%dT%H:%M:%S.%fZ')

                        # Post text content (need to convert HTML string to plain text)
                        row_text = row.xpath('.//div[contains(@class, "_1FwZr") and contains(@class, "_81_1w")]').get()
                        ...
                        post['media_video'] = media_element.xpath('.//div/div/@src').get()

                        # Post engagements (reactions, replies, reposts, quotes)
                        engagement_row = row.xpath('.//div[@class="_UuSG _3dGg1 _2VJFi SslQJ _2pVfg"]')
                        engagements = engagement_row.xpath('.//span[contains(@class, "RiX17")]')
                        engagement_list = []
                        for index, engagement in enumerate(engagements):
                            # Need to convert HTML string to plain text
                            engagement_text = engagement.get()
                            post_engagement = html2text(engagement_text) if engagement_text else None
                            if index == 0:
                                if post_engagement:
                                    post_engagement = f'{post_engagement} reactions'
                                else:
                                    post_engagement = ''
                            engagement_list.append(post_engagement.replace('\n', ''))

                        post['engagement'] = engagement_list
                        user_posts.append(post)

                # Remove duplicates from the list
                distinct_user_posts = [i for n, i in enumerate(user_posts) if i not in user_posts[n + 1:]]

                # Sort distinct list by datetime (recent posts)
                distinct_user_posts_sorted = sorted(distinct_user_posts, key=lambda x: x['datetime'], reverse=True)

                # Harvest up to 50 user posts at maximum
                user_posts_limit = get_project_settings().get('GAB_USER_POST_LIMIT')
                if len(distinct_user_posts_sorted) < user_posts_limit:
